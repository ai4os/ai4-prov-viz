...
export function createD3Graph(
	...
	let svgLinks: d3.Selection<SVGLineElement, D3Link, SVGGElement, undefined> =
		svg.append('g').selectAll('path').attr('marker-end', 'url(#arrowhead)');

	let svgNodes: d3.Selection<SVGGElement, D3Node, SVGGElement, undefined> = svg
		.append('g')
		.selectAll('g');

	const drag = (simulation: D3Simulation) => {
